...
abstract class MSP_LTS2_Model_Rule_Cache
{
    ...
    protected function _getSessionBlocks()
    {
        if (!$this->_sessionBlocks) {
            $sessionBlocks = Mage::getStoreConfig(
                'MSP_LTSR2/settings/sessionblocks',
                Mage::helper('msp_lts2')->getStoreId()
            );
    ...
    protected function _getNoCacheBlocks()
    {
        if (!$this->_noCacheBlocks) {
            $nocacheBlocks = Mage::getStoreConfig(
                'MSP_LTSR2/settings/nocacheblocks',
                Mage::helper('msp_lts2')->getStoreId()
            );
    ...
    protected function _getSessionActions()
    {
        if (!$this->_sessionActions) {
            $sessionActions = Mage::getStoreConfig(
                'MSP_LTSR2/settings/sessionactions',
                Mage::helper('msp_lts2')->getStoreId()
            );
